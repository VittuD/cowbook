...
The SAM3 benchmark image runs `tools.benchmark_sam3_semantic_tracking` for text-only all-instance concept tracking, writes per-video JSON summaries, and produces annotated overlay videos for visual inspection. Its defaults target `/scratch/vet/var/benchmarks/sam3_semantic_tracking_300s`. Per the Ultralytics SAM3 docs, the `sam3.pt` weights are not auto-downloaded and must be provided explicitly in the image or working directory. The image also preinstalls the Ultralytics `CLIP` dependency that SAM3 otherwise attempts to install dynamically at runtime.
