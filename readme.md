...
Build and run the default image:

```bash
docker build -f docker/Dockerfile -t cowbook .
docker run --rm -it -v "$(pwd)/var:/app/var" cowbook
```

The repo also ships benchmark, SAM3, DEVA, and TensorRT images under `docker/`. See [Docker](docs/docker.md) for the full image list, build/run commands for each, and GPU/mount notes.
...
Optional tracking cleanup can preprocess detections before tracking, preserve detection lineage, prune short-lived tracks by gap-tolerant consecutive streak plus optional total-observation threshold, gap-fill small tracking holes, and smooth final output boxes. It is off by default.
